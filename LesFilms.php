<div class="container" style='margin-top:70px; ' >
       <h3 class="font-weight-bold text-center text-uppercase mb-4">Les Films</h3>

<div>
<?php 
$con=Conn();
$req1=$con->prepare('SELECT COUNT(*)AS NB_FILMS FROM document,film WHERE document.NO_DOCUMENT=film.NO_DOCUMENT ORDER BY Date Desc');
$req1->execute();
$ligne=$req1->fetch();
//pagination
$nbfilms= $ligne['NB_FILMS'];
$parpage='6';
...
$req=$con->prepare("SELECT *,DATE_FORMAT(Date, '%e %M %Y') AS DATEMODIF FROM document,film WHERE document.NO_DOCUMENT=film.NO_DOCUMENT ORDER BY Date Desc LIMIT $premier,$parpage");
$req->execute();
while($ligne=$req->fetch()){
	$nodocument=$ligne['NO_DOCUMENT'];
    	echo"<div class='form'>";
		echo "<a type='submit' style='text-decoration-color:black;text-decoration:none;display:flex;margin-bottom:10px;font-size: 20px;' class='btn btn-secondary' href='index.php?page=unFilm&id=".$nodocument."'>";	
